...
class Blockchain {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (await block.validate()) {
          ...
          const validateChainLog = await self.validateChain();
          if (validateChainLog.length === 0) {
            resolve(block);
          } else {
            reject(validateChainLog);
          }
        } else {
          resolve(null);
        }
  ...
  submitStar(address, message, signature, star) {
    let self = this;
    return new Promise(async (resolve, reject) => {
      const timeSubmit = parseInt(message.split(":")[1]);
      ...
      if (currentTime - timeSubmit < FIVEMINS) {
        if (bitcoinMessage.verify(message, address, signature)) {
          try {
            ...
            let result = await self._addBlock(block);
        ...
        reject("The Signature is outdated.");
      }
    });
  }
